...
you can import this file and its functions directly:
    ...
    - jupyter notebook: include the lines `%autoreload 2` and `import preprocess`(or import checkpoint1b)
                        then just call preprocess.remove_percents(df) to test (or checkpoint1b.remove_percents(df))
                        
    - python terminal: run `from importlib import reload` and `import preprocess`(or import checkpoint1b)
                       each time you modify this file, run `reload(preprocess)`(or reload(checkpoint1b))
